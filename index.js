 ...
 */
function extractTextInParentheses(str) {const regex1 = /\((.*?)\)/g; 
const array = [...str.matchAll(regex1)]; const result = [];
array.forEach((arr) => (result[result.length] = arr[1]));
return result;
 ...
 */
function findWordOccurrences(str, word) { const regexp = RegExp(word, "gi");
const matches = [];
let match;
 ...
 */
function checkRegexFlags(regex) {  const arrFlag = [...regex.flags ]; 
  if(arrFlag.includes('g') === true, arrFlag.includes('m') ===true){ return true;} else {return false;}
 ...
 */
function checkFlags(regex) { const arr = [];
  if (regex.flags.includes("i")) arr.push("ignoreCase");
 ...
 */
function checkRegexMethods(regex) {const flagsArr = [];
